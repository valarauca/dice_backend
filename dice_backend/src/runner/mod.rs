...
fn test_nontrivial_program() {
    ...
    use super::parser_output::{AbstractSyntaxTree, Literal, TypeData,Operation};
    use super::cfgbuilder::{HashedExpression, ExpressionCollection};
...
"#;
    ...
    let (left, right) = match coll.get_expr(&return_expr_args[2]) {
        Option::Some(InlinedExpression::Operation(ref left, Operation::Equal, ref right)) => (left.clone(), right.clone()),
        anything_else => panic!("expected a streaming filter operation, found: {:?}", anything_else),
    };
