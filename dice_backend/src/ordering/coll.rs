...
pub use super::expr::{OrderedExpression,OrderingOp,Ordering};

use super::super::runner::{InlinedExpression,InlinedCollection};
...
impl<'a> OrderingCollection<'a> {
    ...
    fn update_order(&mut self) {
        ...
        for (sync,source) in args.iter() {
            let source_order = self.expr.get(source).map(|expr| expr.get_ordering()).unwrap();
            let sync_order = self.expr.get(sync).map(|expr| expr.get_ordering()).unwrap();
            match source_order.cmp(&sync_order) {
                Ordering::Equal | Ordering::Less => {
                    self.expr.get_mut(source).unwrap().set_ordering(sync_order+1);
                },
                _ => { },
            }
        }
